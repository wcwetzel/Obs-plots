...
print(p2)
ggsave(filename='~/Documents/Analysis repos/Obs plots/figs/plot2-time.ps',
	plot=p2, width=3.5, height=3)

m1 = lm(galls2011 ~ galls2010, data=d2)
m1 = mle2(galls2011 ~ dnorm(mean=a + b * galls2010, sd=sd), 
	start=list(a=2, b=1, sd=1), data=d2)
...
newg = 0:37
pred.m2 = data.frame(galls2010 = newg, galls2011 = coef(m2)['a'] + coef(m2)['b'] * newg)
...
p2 = ggplot(data=d2, aes(x=galls2010, y=galls2011)) +
	...
	scale_y_continuous('Galls in 2011') +
	geom_point(alpha = 0.35) +
	geom_smooth(aes(x=galls2010, y=galls2011, ymin=ymin, ymax=ymax), data=pred.m2,
		stat='identity', colour=1, alpha=0.2) +
	theme_bw() +
	opts( panel.grid.minor=theme_blank(), panel.grid.major=theme_blank(),
	axis.title.x = theme_text(vjust = 0))
print(p2)
ggsave(filename='~/Documents/Analysis repos/Obs plots/figs/plot2-time.pdf',
	plot=p2, width=3.5, height=3)
...
pred.msize1 = data.frame(vol = newvol, galls2011 = coef(msize1)['a'] + coef(msize1)['b'] * newvol)
...
p2size = ggplot(data=d2, aes(x=vol, y=galls2011)) +
	...
	geom_point(alpha = 0.35) +
	geom_smooth(aes(x=vol, y=galls2011, ymin=ymin, ymax=ymax), data=pred.msize1,
		stat='identity', colour=1, alpha=0.2) +
	theme_bw() +
	opts( panel.grid.minor=theme_blank(), panel.grid.major=theme_blank(),
	axis.title.x = theme_text(vjust = 0))
print(p2size)
ggsave(filename='~/Documents/Analysis repos/Obs plots/figs/plot2-size.pdf',
...
# mean predictions and CI for msize1
newvol = seq(0,0.9,length=50)
pred.msizec3 = data.frame(vol = newvol, density = coef(msizec3)['a'] + coef(msize1)['b'] * newvol)
...
p2sizec = ggplot(data=d2, aes(x=vol, y=density)) +
	...
	axis.title.x = theme_text(vjust = 0))
print(p2sizec)
ggsave(filename='~/Documents/Analysis repos/Obs plots/figs/plot2-sizec.pdf',
